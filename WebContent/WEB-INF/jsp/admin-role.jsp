			...
			<article class="cl pd-20">
				<div class="cl pd-5 bg-1 bk-gray">
					<span class="l"> <a href="javascript:;" onclick="datadel()"
						...
						onclick="admin_role_add('添加角色','gaddrole','800')"><i
					...
					<table
						...
						<thead>
							...
							<tr class="text-c">
								...
								<th width="40">ID</th>
								<th>角色名</th>
								<th width="200">是否管理员</th>
								<th width="200">是否会员</th>
								<th width="200">微信比率</th>
								<th width="200">支付宝比率</th>
								<th width="70">操作</th>
							</tr>
						</thead>
						<tbody>
							<%
								for (TRole role : roles) {
									String isAdmin = role.getIsAdmin() == 1 ? "是" : "否";
									String isMember = role.getIsMember() == 1 ? "是" : "否";
							%>
							<tr class="text-c">
								...
								<td><%=isAdmin%></td>
								<td><%=isMember%></td>
								<td><%=role.getWxRate()%></td>
								<td><%=role.getAliRate()%></td>
								<td class="f-14"><a title="编辑" href="javascript:;"
									onclick="admin_role_edit('角色编辑','gaddrole?id=<%=role.getId()%>','1')"
									style="text-decoration: none"><i class="Hui-iconfont">&#xe6df;</i></a>
									<a title="删除" href="javascript:;"
									onclick="admin_role_del(this,'<%=role.getId()%>')" class="ml-5"
	...
	<script type="text/javascript">
		...
		function admin_role_del(obj, id) {
			layer.confirm('角色删除须谨慎，确认要删除吗？', function(index) {
				window.location.href = "delrole?id=" + id;
			});
		}
	</script>
	<!--/请在上方写此页面业务相关的脚本-->
